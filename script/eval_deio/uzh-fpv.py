...
from utils.load_utils import load_gt_us,fpv_evs_iterator, fpv_evs_h5_iterator
...
if __name__ == '__main__':
    ...
    if cfg.ENALBE_IMU:
        print("\033[41m with IMU \033[0m ")
    else:
        print("\033[41m no IMU \033[0m ")
    ...
    for i, scene in enumerate(test_scenes):
        ...
        groundtruth = os.path.join(args.inputdir, scene, f"stamped_groundtruth_us.txt")#真值的路径，注意这是有时间偏移的真值
        imupath = os.path.join(args.inputdir, scene, f"imu.txt")#IMU的路径
        ...
        for trial in range(args.trials):
            ...
            #                             iterator=fpv_evs_h5_iterator(datapath_val, stride=args.stride, timing=False, H=260, W=346,tss_gt_us=tss_traj_us),
            #                             timing=False, H=260, W=346, viz_flow=False)
            
            # elif cfg.ENALBE_IMU:#如果开启了IMU
            if cfg.ENALBE_IMU or cfg.LOOP_CLOSURE:#如果有IMU或者GBA
                ...
                """ Load IMU data """
                all_imu = np.loadtxt(imupath, delimiter=' ', usecols=range(1, 8))#去掉第0列序号
                #将IMU的时间戳转换为微妙
                all_imu[:,0] *= 1e6 #读入的imu时间是秒，转换为微秒
                #将IMU的时间减去偏移量
                ...
                traj_est, tstamps, flowdata, _ = run_DEIO2(datapath_val, cfg, args.network, viz=args.viz, 
                                        iterator=fpv_evs_h5_iterator(datapath_val, stride=args.stride, timing=False, H=260, W=346,tss_gt_us=tss_traj_us),
                                        _all_imu=all_imu,
                                        _all_gt=all_gt,
                ...
                traj_est, tstamps, flowdata, _ = EVO_run(datapath_val, cfg, args.network, viz=args.viz, 
                                        iterator=fpv_evs_h5_iterator(datapath_val, stride=args.stride, timing=False, H=260, W=346,tss_gt_us=tss_traj_us),
                                        timing=False, H=260, W=346, viz_flow=False)
